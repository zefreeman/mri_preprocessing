...
nsubs = length(subs);
nsruns = 3; % Number of runs per session - this being hard coded may be an issue for the ones where there's one or two
...
%mnames_compcors = {'c_comp_cor_00', 'c_comp_cor_01', 'c_comp_cor_02', 'c_comp_cor_03', 'c_comp_cor_04', ...
%               'w_comp_cor_00', 'w_comp_cor_01', 'w_comp_cor_02', 'w_comp_cor_03', 'c_comp_cor_04'};
...
for sub = 1:nsubs
    try
        % loop over subjects
            subdata = [dataroot filesep subs{sub} '/ses-mri01/func/'];
            cd(subdata);
            mkdir analysis
            cd analysis
            delete SPM.mat 
            cd(subdata)
            clear matlabbatch
            matlabbatch{1}.spm.stats.fmri_spec.dir = cellstr([subdata 'analysis']); %% 
            ...
            for i = 1:nsruns
                % Check if movement files are there for the current run
                        if incmoves == 1
                            ...
                            moves = []; % does this need to happen any more?
                            ...
                            % Read each movement file and extract static columns
                                ...
                                R=rp; %this fixed a problem 20240813 that I did not understand
                                eval(['save movevals_run' num2str(i) '.mat rp R']); % this needs to happen because it's not saving from the above

            % Grab - data, onsets and movement
             imgfilt = ['^s6sub-*.*_ses-mri01_task-TRMrun' num2str(i) '_space-MNI152NLin2009cAsym_desc-preproc_bold.nii$']; % Image filter - s6 for smoothed 6mm
             files = spm_select('ExtFPList', subdata, imgfilt);
             onsetsfilt = sprintf('.*_run%d_bh\\.mat$', i);  % 
             onsetsfile = spm_select('FPList', pwd, onsetsfilt);
             load(onsetsfile)

            matlabbatch{1}.spm.stats.fmri_spec.sess(i).scans = cellstr(files);
            
            for k = 1:length(onsets)
                matlabbatch{1}.spm.stats.fmri_spec.sess(i).cond(k).name =names{k}; %% here? "
                matlabbatch{1}.spm.stats.fmri_spec.sess(i).cond(k).onset =onsets{k}; %% here? this seems to work ok when looking at the batch output
                matlabbatch{1}.spm.stats.fmri_spec.sess(i).cond(k).duration =durations{k};
                matlabbatch{1}.spm.stats.fmri_spec.sess(i).cond(k).tmod = 0;
                if k ==1
                matlabbatch{1}.spm.stats.fmri_spec.sess(i).cond(k).pmod.name = ' own ratings'
                matlabbatch{1}.spm.stats.fmri_spec.sess(i).cond(k).pmod.param =pmod.OwnRating
                matlabbatch{1}.spm.stats.fmri_spec.sess(i).cond(k).pmod.poly = 1
                elseif k ==3
                    
                end
                matlabbatch{1}.spm.stats.fmri_spec.sess(i).cond(k).orth = 1;
            end
        matlabbatch{1}.spm.stats.fmri_spec.sess(i).multi = {''};
        matlabbatch{1}.spm.stats.fmri_spec.sess(i).regress = struct('name', {}, 'val', {});
        matlabbatch{1}.spm.stats.fmri_spec.sess(i).multi_reg = cellstr([pwd filesep 'movevals_run' num2str(i) '.mat']) ;
        matlabbatch{1}.spm.stats.fmri_spec.sess(i).hpf = 74;
        end
        ...
        matlabbatch{3}.spm.stats.con.consess{1}.tcon.sessrep = 'replsc'; %% this is replicating it over the three runs so the contrasts are re-selected each time 
