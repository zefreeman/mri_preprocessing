...
for i = 1:length(subs)
    ...
    % Adjust the folder path as needed for your data organization
    ...
    for r = 1:length(roi_files)
        ...
        % Load group-level ROI mask (unaltered)
        roi_nii = spm_vol(roi_path);
        roi_data = spm_read_vols(roi_nii);
        roi_bin = roi_data > 0;
        
        % If dimensions do not match, reslice subject mask into ROI space using nearest neighbor interpolation
        if ~isequal(size(roi_bin), size(subj_bin))
            fprintf('Dimensions differ for ROI %s and subject %s. Reslicing subject mask to ROI space with nearest neighbor interpolation...\n', roi_files{r}, subj);
            % Create a two-row char array where the first row is the reference (ROI) and
            % the second row is the subject mask to be resliced.
            P = char(roi_path, mask_path);
            spm_reslice(P, struct('mean', false, 'interp', 0));  % Use nearest neighbor (interp=0)
            % The resliced subject mask is saved with prefix 'r'
            [p, n, ext] = fileparts(mask_path);
            subj_resliced_path = fullfile(p, ['r' n ext]);
            if exist(subj_resliced_path, 'file') ~= 2
                warning('Resliced subject mask file not found: %s, skipping.', subj_resliced_path);
                continue;
            end
            subj_nii = spm_vol(subj_resliced_path);
            subj_data = spm_read_vols(subj_nii);
            subj_bin = subj_data > 0;
        end
...
disp(T);
csv_file = fullfile(dataroot, 'roi_overlap3_all_subjects_M1.csv');
