...
for sub = 1:nsubs
    try
        ...
        for run = 1:nruns
            ...
            onsdurfile_pattern = sprintf('*_onsets_durations_run%d.mat', run);
            onsdurfile = spm_select('List', sessdata, onsdurfile_pattern);
            mat_contents = load(fullfile(sessdata, onsdurfile));

            % Extract onsets and durations from the loaded .mat file
            onsets = mat_contents.onsets;
            durations = mat_contents.durations;

            tc = tc + 1;
            for c = 1:ncond
                SPM.Sess(tc).U(c) = struct('ons', onsets{c}, 'dur', durations{c}, 'name', {cnames{1}(c)}, 'P', struct('name', 'none'));
            end
