...
for i = 1:length(subs)
    ...
    for r = 1:length(roi_files)
        ...
        % If dimensions do not match, reslice ROI to subject space using nearest neighbor interpolation
        if ~isequal(size(roi_bin), size(subj_bin))
            fprintf('Dimensions differ for ROI %s and subject %s. Reslicing ROI with nearest neighbor interpolation...\n', roi_files{r}, subj);
            % Create a two-row char matrix where first row is the reference (subject mask) 
            % and second row is the ROI to be resliced.
            P = char(mask_path, roi_path);
            % Set 'interp' to 0 (nearest neighbor) and 'mean' to false
            spm_reslice(P, struct('mean', false, 'interp', 0));  
