...
folder="/data/project/STAR/STAR_preprocessed/N_fmriprep/data/derivatives/sub-ID48/ses-mri01/spike_test/"

# Loop through all files matching the pattern *task*_desc-confounds_timeseries.tsv
for file in "$folder"*task*_desc-confounds_timeseries.tsv; do
    ...
    if [ -f "$file" ]; then
        ...
        if [ -n "$fw_index" ]; then
            ...
            # Remove the spike_2 column if it exists
            awk 'BEGIN{FS=OFS="\t"} {
                if (NR==1) {
                    for (i=1; i<=NF; i++) {
                        if ($i == "spike_1") {
